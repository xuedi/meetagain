...
A plugin can extend the public REST API at `/api/*` by:

1. Returning a fragment from `Plugin::getOpenApiFragment()` so the endpoint is documented
   on hosts where the plugin is active.
2. Shipping a controller under `plugins/<key>/src/Controller/NonLocale/Api/` that handles
   the route.
3. Calling `App\Service\Api\PluginRouteGuard::requireActive()` at the top of every plugin
   action so the controller 404s on hosts where the plugin is disabled.

### OpenAPI fragment file

Co-locate the fragment at `plugins/<key>/config/api/openapi-fragment.php` and load it from
the plugin kernel:
...
The fragment is merged into the public spec by `OpenApiSpecBuilder`. Path, schema, and tag
collisions surface as `OpenApiCollisionException` rather than silently overwriting.

### Per-domain scoping

`OpenApiSpecBuilder` filters fragments by `PluginService::getActiveList()` for the current
request. A fragment is merged only if its `getPluginKey()` is in that active list - so on a
host where the plugin is disabled, the path is absent from `paths`, and `PluginRouteGuard`
returns 404 to any stale link that hits the controller. There is nothing extra to wire; just
implement `getPluginKey()` (which the base interface already requires).

### Cache pattern

Read endpoints should pull through `App\Service\Api\ApiCache` for short-TTL caching. The
helper bypasses the cache for any non-GET method and for any request with an `Authorization`
header, so authenticated traffic always sees fresh data:

```php
$payload = $this->apiCache->getJson(
    $request,
    'myplugin.list',          // endpoint key
    600,                      // TTL seconds
    ['locale' => $locale],    // vary parameters
    fn(): array => $this->buildPayload($locale),
);
```

Cache keys include the request host and a fingerprint of the active plugin set, so cached
payloads do not bleed across domains or per-group activation toggles.

### Security

For public endpoints, add a `PUBLIC_ACCESS` rule to `config/packages/security.yaml`. Token-
gated endpoints fall through to the default firewall - no rule is needed, but the controller
must expect an authenticated user.
