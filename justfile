...
# Installer test stack configuration
INSTALLER_DOCKER := "docker-compose -f docker/installer-test/docker-compose.yml"

# Start the installer test stack (nginx + php-fpm + mariadb on port 8080)
[group('installer-test')]
installerStart:
    @echo "Building and starting installer test stack..."
    {{INSTALLER_DOCKER}} up -d --build
    @echo ""
    @echo "Installer test stack is running!"
    @echo "Access: http://localhost:8080/install/"
    @echo ""
    @echo "Database credentials for installer:"
    @echo "  Host: ma-installer-db"
    @echo "  Database: meetagain_test"
    @echo "  User: meetagain"
    @echo "  Password: testpass"
    @echo "  Root Password: rootpass"

# Stop and remove the installer test stack (keeps volumes)
[group('installer-test')]
installerStop:
    {{INSTALLER_DOCKER}} down
    @echo "Installer test stack stopped"

# Completely remove installer test stack including volumes and built images
[group('installer-test')]
installerClean:
    {{INSTALLER_DOCKER}} down -v --rmi local
    rm -f .env var/installed.lock
    @echo "Installer test stack removed completely"
    @echo "Deleted .env and var/installed.lock for fresh install test"

# View logs from installer test stack
[group('installer-test')]
installerLogs:
    {{INSTALLER_DOCKER}} logs -f

# Open shell in installer test PHP container
[group('installer-test')]
installerShell:
    {{INSTALLER_DOCKER}} exec php bash