...
final class Version20260702120000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->connection->executeStatement(
            'ALTER TABLE event ADD CONSTRAINT FK_3BAE0AA75278319C FOREIGN KEY (series_id) REFERENCES event_series (id) ON DELETE SET NULL',
        );
        ...
        $childMissed = (int) $this->connection->fetchOne('SELECT COUNT(*) FROM event e JOIN event e2 ON e2.id = e.recurring_of WHERE e.series_id IS NULL');
        $this->abortIf($childMissed > 0, sprintf('%d child events with a living parent missed the series backfill', $childMissed));
        $parentMissed = (int) $this->connection->fetchOne('SELECT COUNT(*) FROM event WHERE recurring_rule IS NOT NULL AND series_id IS NULL');
