...
$DOCKER exec -T mariadb sh -c "mariadb-dump --user='${MARIADB_USER}' --password='${MARIADB_PASSWORD}' --no-tablespaces --single-transaction --quick '${template}' > '${dump_path}'"

printf 'Cloning into %d per-process DBs ...\n' "$procs"
i=1
while [ "$i" -le "$procs" ]; do
    clone="${template}${i}"
    $DOCKER exec -T mariadb sh -c "
...
$DOCKER exec -T mariadb sh -c "rm -f '${dump_path}'"
