...
use App\Security\Permission\Gateway;
use Override;
...
class GatewayTest extends TestCase
{
    ...
    public function testContextCarriesActorAndAdminFlag(): void
    {
        ...
        $user = new User();
        $captured = null;
        $checker = new class($captured) implements CheckerInterface {
            public function __construct(
                public ?Context &$captured,
            ) {}
    ...
    private function makeGateway(bool $isAdmin, CheckerInterface ...$checkers): Gateway
    {
        $security = $this->createMock(Security::class);
        $security->method('isGranted')->willReturn($isAdmin);

        return new Gateway($security, $checkers);
    }
    ...
    private function fixedChecker(bool $supports, ?bool $decision): CheckerInterface
    {
        return new class($supports, $decision) implements CheckerInterface {
            public function __construct(
