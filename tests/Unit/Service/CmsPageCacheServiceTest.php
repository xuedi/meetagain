...
class CmsPageCacheServiceTest extends TestCase
{
    ...
    public function testStoreInvokesCallbackWithCorrectTag(): void
    {
        ...
        $cacheMock->expects($this->once())
            ->method('get')
            ->with(
                $this->stringContains('cms_page.7.'),
                $this->isCallable(),
                \INF,
            )
            ->willReturnCallback(static function (string $key, callable $callback, float $beta) use (&$capturedTag): string {
                $item = new class implements ItemInterface {
                    private array $tags = [];
                    public function getKey(): string { return ''; }
                    public function get(): mixed { return null; }
                    ...
                    public function expiresAt(?\DateTimeInterface $expiration): static { return $this; }
                    public function expiresAfter(int|\DateInterval|null $time): static { return $this; }
                    public function tag(string|iterable $tags): static { $this->tags = is_string($tags) ? [$tags] : [...$tags]; return $this; }
                    public function getMetadata(): array { return []; }
