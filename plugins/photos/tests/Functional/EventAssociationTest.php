...
{
    private const int FIRST_EVENT = 20;
    private const int SECOND_EVENT = 21;
    ...
    {
        // Arrange
        $photo = $this->photoOf(self::SECOND_EVENT);
        $this->inScope(fn(): null => $this->writer->setEvent($photo, $this->event(self::FIRST_EVENT)));
        $first = $this->inScope(fn(): ?ItemTag => $this->dateTagService->findDateTag($this->event(self::FIRST_EVENT)));
        static::assertNotNull($first);

        // Act
        $this->inScope(fn(): null => $this->writer->setEvent($photo, $this->event(self::SECOND_EVENT)));

        // Assert
        static::assertSame([self::SECOND_EVENT], $this->eventIds($photo));
        static::assertNull($this->inScope(fn(): ?ItemTag => $this->dateTagService->findDateTag($this->event(self::FIRST_EVENT))));
        static::assertNotNull($this->inScope(fn(): ?ItemTag => $this->dateTagService->findDateTag($this->event(self::SECOND_EVENT))));
    }
    ...
    public function testSavingTheSameEventAgainChangesNothing(): void
    {
        ...
        $photo = $this->photoOf(self::SECOND_EVENT);
        $this->inScope(fn(): null => $this->writer->setEvent($photo, $this->event(self::FIRST_EVENT)));
        $tag = $this->inScope(fn(): ?ItemTag => $this->dateTagService->findDateTag($this->event(self::FIRST_EVENT)));

        // Act
        $this->inScope(fn(): null => $this->writer->setEvent($photo, $this->event(self::FIRST_EVENT)));

        // Assert
        static::assertSame([self::FIRST_EVENT], $this->eventIds($photo));
        static::assertSame($tag?->getId(), $this->inScope(fn(): ?ItemTag => $this->dateTagService->findDateTag($this->event(self::FIRST_EVENT)))?->getId());
    }

    public function testTheDateRowSurvivesWhileAnotherPhotoStillCarriesIt(): void
    {
        // Arrange
        $photos = $this->photosOf(self::SECOND_EVENT, 2);
    ...
    {
        return $this->eventScope->runForEvent(self::SECOND_EVENT, $work);
    }
