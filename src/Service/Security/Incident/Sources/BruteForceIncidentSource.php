...
/**
 * Reads login_throttling rate-limit hits. Threshold is 1: any login throttle
 * is worth surfacing as an incident signal.
 */
final readonly class BruteForceIncidentSource implements IncidentSourceInterface
{
    public const string KEY = IncidentSourceContribution::KEY_BRUTE_FORCE;
    public const string KEY_LAST_PROCESSED_ID = 'security.incident.brute_force.last_processed_log_id';
    ...
    public function ingest(): IncidentSourceStats
    {
        ...
        $rows = $this->repo->findRowsAfterIdUpToByLimiter($lastId, $cutoff, self::BATCH_SIZE, self::LOGIN_LIMITER, exclude: false);
        if ($rows === []) {
