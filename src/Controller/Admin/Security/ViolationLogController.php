...
final class ViolationLogController extends AbstractSecurityController implements AdminNavigationInterface, AdminTabsInterface
{
    ...
    #[Route('/admin/security/permissions/violations', name: 'app_admin_security_permissions_violations', methods: ['GET'])]
    public function list(Request $request): Response
    {
        ...
        $info = [
            new AdminTopInfoHtml(sprintf('<strong>%d</strong>&nbsp;%s', $totalCount, $this->translator->trans('admin_security.summary_total_access_denied'))),
        ];
        if ($rangeCount === 0) {
        ...
        return $this->render('admin/security/violation_log.html.twig', [
            'active' => 'security',
            'rows' => $recent,
            'adminTop' => $adminTop,
    ...
    private function buildRangeDropdown(string $current): AdminTopActionDropdown
    {
        ...
        foreach (array_keys(self::RANGE_OFFSETS) as $key) {
            ...
            $options[] = new AdminTopActionDropdownOption(
                ...
                target: $this->generateUrl('app_admin_security_permissions_violations', $params),
